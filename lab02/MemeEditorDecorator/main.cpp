...
#include "httplib.h"
#include "components/memeimage.h"
#include "components/textdecorator.h"
#include "components/filterdecorator.h"
#include "components/borderdecorator.h"
#include "components/stickerdecorator.h"
#include <iostream>
#include <sstream>

MemeComponent* currentMeme = nullptr;
...
int main() {
    ...
    svr.Post("/api/meme", [](const httplib::Request& req, httplib::Response& res) {
        ...
        if (!currentMeme) {
            currentMeme = new MemeImage("cat.jpg");
        }
        ...
        if (type == "text") {
            ...
            currentMeme = new TextDecorator(currentMeme, text, pos, size, color);
        } else if (type == "filter") {
            std::string filter = getJsonValue(body, "filterType");
            currentMeme = new FilterDecorator(currentMeme, filter);
        } else if (type == "border") {
            int width = getIntValue(body, "width", 50);
            std::string color = getJsonValue(body, "color");
            currentMeme = new BorderDecorator(currentMeme, width, color);
        } else if (type == "sticker") {
            std::string path = getJsonValue(body, "stickerPath");
            std::string pos = getJsonValue(body, "position");
            currentMeme = new StickerDecorator(currentMeme, path, pos);
        }
    });
