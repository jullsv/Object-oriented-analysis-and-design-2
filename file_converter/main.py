...
from visitors.convert_to_jpeg import ConvertToJPEGVisitor
from visitors.convert_to_png import ConvertToPNGVisitor
from visitors.convert_to_pdf import ConvertToPDFVisitor

class FileConverterGUI:
    def __init__(self, root):
        self.root = root
        self.root.title("Конвертер файлов (Pattern)")
    ...
    def _convert(self, format: str):
        ...
        if format == "jpeg":
            visitor = ConvertToJPEGVisitor(output, quality=90)
        elif format == "png":
            visitor = ConvertToPNGVisitor(output, compression=6)
        elif format == "pdf":
            visitor = ConvertToPDFVisitor(output, page_size="A4")
        
        self._log(f"Запуск конвертации в {format.upper()}")
        
        self.structure.accept(visitor)
